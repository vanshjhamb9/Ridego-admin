...
async function ZoneLatlon(pickup_lat_lon, drop_lat_lon, lat_lon_list) {
    ...
    // Handle empty array or empty string check
    if (lat_lon_list && lat_lon_list != "" && Array.isArray(lat_lon_list) && lat_lon_list.length > 0) {
...
async function ZoneData() {
    ...
    for (let i = 0; i < zone_data.length;) {
        let lat_lon_str = zone_data[i].lat_lon || '';
        let all_lat = [];
        
        // Handle different zone formats:
        // Format 1: "lat1,lon1:lat2,lon2:lat3,lon3" (colon-separated pairs)
        // Format 2: "lat1,lon1" (single point)
        
        if (lat_lon_str && lat_lon_str.trim() !== '') {
            // Check if it contains colon (multiple points format)
            if (lat_lon_str.includes(':')) {
                // Split by colon to get pairs
                let pairs = lat_lon_str.split(':');
                for (let a = 0; a < pairs.length; a++) {
                    let pair = pairs[a].trim();
                    if (pair) {
                        let coords = pair.split(',');
                        if (coords.length >= 2) {
                            let latitude = Number(coords[0].trim());
                            let longitude = Number(coords[1].trim());
                ...
                // Single point format: "lat,lon"
                let coords = lat_lon_str.split(',');
                if (coords.length >= 2) {
                    let latitude = Number(coords[0].trim());
                    let longitude = Number(coords[1].trim());
                    if (!isNaN(latitude) && !isNaN(longitude)) {
        ...
        // Ensure at least one point exists, otherwise skip this zone
        if (all_lat.length > 0) {
...
async function CheckZone(convertzone, zone, zalldata) {
    ...
    if (!convertzone || !Array.isArray(convertzone) || convertzone.length === 0) {
        return [{ zr: [] }, { zc: 1 }, { zid: [] }]; // Return error state
    }

    if (!all_zone || !Array.isArray(all_zone) || all_zone.length === 0) {
        return [{ zr: [] }, { zc: 1 }, { zid: [] }]; // Return error state - no zones
    }

    for (let a = 0; a < convertzone.length; a++) {
        let tcount = 0, ucheck = 0;
        
        // Validate point has latitude and longitude
        if (!convertzone[a] || convertzone[a].latitude === undefined || convertzone[a].longitude === undefined) {
            console.log("Invalid point in convertzone:", convertzone[a]);
            uzone++;
        ...
        let pointLat = Number(convertzone[a].latitude);
        let pointLon = Number(convertzone[a].longitude);
        
        if (isNaN(pointLat) || isNaN(pointLon)) {
            console.log("Invalid latitude/longitude:", convertzone[a]);
            uzone++;
            zoneresult.push({ zone: a + 1, status: 0 });
            continue;
        }

        let validPoint = { latitude: pointLat, longitude: pointLon };

        for (let b = 0; b < all_zone.length; b++) {
            // Validate zone polygon exists and has points
            if (!all_zone[b] || !Array.isArray(all_zone[b]) || all_zone[b].length === 0) {
            ...
            try {
                ...
                // If zone is a single point, use radius check (50km default, or 100km for larger coverage)
                if (all_zone[b].length === 1) {
                    // Single point zone - use radius check
                    // Default radius: 50km (50,000 meters) - can be adjusted based on zone size
                    const zoneRadius = 50000; // 50km in meters
                    const zoneCenter = all_zone[b][0];
                    
                    if (zoneCenter && zoneCenter.latitude !== undefined && zoneCenter.longitude !== undefined) {
                        count = geolib.isPointWithinRadius(
                            validPoint,
                            { latitude: Number(zoneCenter.latitude), longitude: Number(zoneCenter.longitude) },
                            zoneRadius
                        );
                    }
                } else if (all_zone[b].length >= 3) {
                    // Polygon zone - use polygon check (needs at least 3 points)
                    count = geolib.isPointInPolygon(validPoint, all_zone[b]);
                } else {
                    // 2 points - treat as single point (use radius)
                    const zoneCenter = all_zone[b][0];
                    const zoneRadius = 50000; // 50km
                    if (zoneCenter && zoneCenter.latitude !== undefined && zoneCenter.longitude !== undefined) {
                        count = geolib.isPointWithinRadius(
                            validPoint,
                            { latitude: Number(zoneCenter.latitude), longitude: Number(zoneCenter.longitude) },
                ...
                console.log("Error in zone check:", geoError, "Point:", validPoint, "Zone:", all_zone[b]);
                ucheck++;
            }
        }
...
async function GetDistance(pickup, drop, google_map_key) {
    ...
    // Parse coordinates if they're in "lat,lon" format
    let pickupCoords = { latitude: 0, longitude: 0 };
    let dropCoords = { latitude: 0, longitude: 0 };
    
    if (typeof origin === 'string' && origin.includes(',')) {
        const [lat, lon] = origin.split(',').map(Number);
        pickupCoords = { latitude: lat, longitude: lon };
    }
    
    if (typeof destination === 'string' && destination.includes(',')) {
        const [lat, lon] = destination.split(',').map(Number);
        dropCoords = { latitude: lat, longitude: lon };
    }
        ...
        try {
            ...
            if (data.status === 'OK' && data.rows && data.rows.length > 0 && data.rows[0].elements && data.rows[0].elements.length > 0) {
                ...
                if (element.status === 'OK') {
                    ...
                    if (dspl[1] && dspl[1].toLowerCase() !== 'km') {
                        kmcal = (parseFloat(dspl[0]) / 1000).toFixed(2);
                    } else {
                        kmcal = parseFloat(parseFloat(dspl[0]).toFixed(2));
                    }

                    return { status: 1, dis: parseFloat(kmcal), dur: duration }
                } else {
                    console.log('Google Maps API error:', element.status);
            ...
            console.log('Error fetching distance from Google Maps API:', error.message);
            // Fall through to fallback calculation
        }
    } else {
    ...
    // FALLBACK: Use geolib to calculate straight-line distance and estimate duration
    try {
        if (pickupCoords.latitude !== 0 && dropCoords.latitude !== 0) {
            // Calculate distance in meters
            const distanceInMeters = geolib.getDistance(pickupCoords, dropCoords);
            // Convert to kilometers
            const distanceInKm = (distanceInMeters / 1000).toFixed(2);
            
            // Estimate duration based on average speed (assuming 30 km/h in city)
            const avgSpeedKmh = 30;
            const durationInMinutes = Math.round((parseFloat(distanceInKm) / avgSpeedKmh) * 60);
            ...
            if (durationInMinutes >= 60) {
                ...
                durationText = minutes > 0 ? `${hours} hour ${minutes} min` : `${hours} hour`;
            } else {
                durationText = `${durationInMinutes} min`;
            }

            console.log(`Using fallback calculation: ${distanceInKm} km, ${durationText}`);
            return { status: 1, dis: parseFloat(distanceInKm), dur: durationText };
        }
    } catch (error) {
        console.log('Error in fallback distance calculation:', error);
    }
