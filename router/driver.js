...
router.get("/list", auth, async (req, res) => {
    try {
        const { subscription_filter } = req.query; // Get filter from query string
        
        let whereClause = "";
        if (subscription_filter) {
            if (subscription_filter === 'active') {
                whereClause = "WHERE (d.subscription_status = 'active' AND d.subscription_end_date IS NOT NULL AND d.subscription_end_date >= NOW())";
            } else if (subscription_filter === 'expired') {
                whereClause = "WHERE d.subscription_status = 'expired' OR (d.subscription_status = 'active' AND d.subscription_end_date IS NOT NULL AND d.subscription_end_date < NOW())";
            } else if (subscription_filter === 'none') {
                whereClause = "WHERE (d.subscription_status = 'none' OR d.subscription_status IS NULL)";
            } else if (subscription_filter === 'expiring_soon') {
                whereClause = "WHERE d.subscription_status = 'active' AND d.subscription_end_date IS NOT NULL AND d.subscription_end_date BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 7 DAY)";
            }
        }
        
        const driver_list = await DataFind(`SELECT d.*, 
                                            ...
                                            CASE 
                                                WHEN d.subscription_status = 'active' AND d.subscription_end_date IS NOT NULL AND d.subscription_end_date >= NOW() 
                                                THEN DATEDIFF(d.subscription_end_date, NOW())
                                                WHEN d.subscription_status = 'active' AND d.subscription_end_date IS NOT NULL AND d.subscription_end_date < NOW()
                                                THEN -1
                                                WHEN d.subscription_status = 'expired' OR (d.subscription_end_date IS NOT NULL AND d.subscription_end_date < NOW())
                                                THEN -1
                                                ELSE 0
                                            ...
                                            CASE 
                                                ...
                                                WHEN d.subscription_status = 'expired' OR (d.subscription_end_date IS NOT NULL AND d.subscription_end_date < NOW())
                                                THEN 'expired'
                                                WHEN d.subscription_status = 'none' OR d.subscription_status IS NULL
                                                THEN 'none'
        ...
        res.render("driver", {
            auth: req.user, general: req.general, noti: req.notification, per: req.per, lan: req.lan.ld, land: req.lan.lname, driver_list, subscription_filter: subscription_filter || 'all'
        })
    } catch (error) {
        console.log(error);
    }
...
router.get("/cash_adjustment/:id", auth, async (req, res) => {
    try {
        // Get driver basic info and cash amounts
        const driver = await DataFind(`SELECT dri.id, dri.first_name, dri.last_name, dri.email, dri.primary_phoneNo, 
                                        dri.tot_cash, dri.wallet, dri.payout_wallet,
                                        COALESCE(SUM(cadj.amount), 0) AS tot_pendinga
                                        FROM tbl_driver AS dri
        ...
        if (driver != "") {
            tot_cash = parseFloat((parseFloat(driver[0].tot_cash || 0) - parseFloat(driver[0].tot_pendinga || 0)).toFixed(2));
            wallet_balance = parseFloat(driver[0].wallet || 0);
            payout_wallet = parseFloat(driver[0].payout_wallet || 0);
        }

        // Get cash adjustment list
        ...
        // Get wallet transaction history - Include all transactions, not just those linked to orders
        const wallet_history = await DataFind(`SELECT 
            td.id, 
            td.payment_id, 
            td.amount, 
            td.date, 
            ...
            CASE 
                ...
                WHEN td.type IS NULL OR td.type = '' THEN 'Ride Earnings'
                ELSE 'Other'
            END AS transaction_type,
            CASE 
                WHEN td.status = '1' THEN 'credit'
                WHEN td.status = '2' THEN 'debit'
                ELSE 'pending'
            END AS transaction_category
            FROM tbl_transaction_driver AS td
            LEFT JOIN tbl_order_vehicle AS ov ON td.payment_id = ov.id
            LEFT JOIN tbl_payment_detail AS pd ON ov.payment_id = pd.id
            WHERE td.d_id = '${req.params.id}' 
            ORDER BY td.id DESC 
            LIMIT 100`);

        wallet_history.map(val => {
            const date = new Date(val.date);
            const formattedDate = date.toLocaleString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone });
            val.date = formattedDate;
            return val;
        ...
        const wallet_stats = await DataFind(`SELECT 
            COALESCE(SUM(CASE WHEN status = '1' THEN amount ELSE 0 END), 0) AS total_earnings,
            COALESCE(SUM(CASE WHEN status = '2' THEN amount ELSE 0 END), 0) AS total_withdrawals,
            COUNT(CASE WHEN status = '1' THEN 1 END) AS total_credits,
            COUNT(CASE WHEN status = '2' THEN 1 END) AS total_debits
        ...
        res.render("driver_cash_adjustment", {
            ...
            lan: req.lan.ld, land: req.lan.lname, cash_list, driver: driver[0] || {}, 
            tot_cash, wallet_balance, payout_wallet, wallet_history, wallet_stats: wallet_stats[0] || {}
        })
    } catch (error) {
        console.log(error);
    }
