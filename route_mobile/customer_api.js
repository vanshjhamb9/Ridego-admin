...
router.post("/home", async (req, res) => {
    try {
        ...
        let package_list = await DataFind(`SELECT id, image, name FROM tbl_package WHERE status = '1'`);
        if (package_list && Array.isArray(package_list) && package_list.length > 0) {
            package_list.map(rvel => {
                rvel.bidding = "0"
                rvel.role = "4"
                rvel.description = ""; // Add empty description if needed by frontend
            });
        } else {
            package_list = []; // Ensure it's an array even if query fails or returns empty
        }
...
router.post("/home_mape", async (req, res) => {
    try {
        ...
        // Validate and convert lat/lon to numbers
        const latitude = Number(lat);
        const longitude = Number(lon);
        
        if (isNaN(latitude) || isNaN(longitude)) {
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid latitude or longitude' });
        }
        
        let convertzone = [{ latitude, longitude }];
        let dzone = await AllFunction.ZoneData();
        
        // Validate zone data
        if (!dzone || !dzone.all_zone || !dzone.zone_data) {
            console.log("Zone data is invalid:", dzone);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Zone data not available' });
        }
        
        let zonecheck = await AllFunction.CheckZone(convertzone, dzone.all_zone, dzone.zone_data);
        
        // Validate zonecheck result
        if (!zonecheck || !Array.isArray(zonecheck) || zonecheck.length < 3) {
            console.log("Zone check returned invalid result:", zonecheck);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Error checking zone' });
        }
...
router.post("/calculate", async (req, res) => {
    try {
        ...
        if (mrole == "1") {
            ...
            // Validate convertzone
            if (!convertzone || !Array.isArray(convertzone) || convertzone.length === 0) {
                return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid location coordinates' });
            }
        ...
        if (mrole == "4") {
            let package_list = await DataFind(`SELECT id, image, name FROM tbl_package WHERE status = '1'`);
            // Handle if package_list is not an array (e.g., query error or empty result)
            if (!package_list || !Array.isArray(package_list)) {
                package_list = [];
            }
        }
...
router.post("/module_calculate", async (req, res) => {
    try {
        ...
        // Validate zonecheck result
        if (!zonecheck || !Array.isArray(zonecheck) || zonecheck.length < 3) {
            console.log("Invalid zonecheck result:", zonecheck);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Error checking zone' });
        }

        if (zonecheck[1] && zonecheck[1].zc && zonecheck[1].zc != "0") {
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Address is not in the zone!' });
        }
        ...
        // Validate zoneresult
        if (!zoneresult || !Array.isArray(zoneresult) || zoneresult.length < 2) {
            console.log("Invalid zoneresult:", zoneresult);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid zone data' });
        }
        
        for (let a = 1; a < zoneresult.length; a++) {
            // Validate convertzone indices
            if (!convertzone || !Array.isArray(convertzone) || convertzone.length < a + 1) {
                console.log("Invalid convertzone at index:", a);
                continue;
            }
            ...
            if (distance && distance.status === 1 && distance.dis > 0) {
                cal += parseFloat(distance.dis);
                ...
                if (spltime.length == 2) {
                    ...
                    dhour = parseFloat(spltime[0]) || 0; 
                    dtime = parseFloat(spltime[2]) || 0;
                } else {
                    // Try to extract number from duration string
                    const timeMatch = distance.dur.match(/(\d+)/);
                    if (timeMatch) {
                        dtime = parseFloat(timeMatch[1]) || 0;
                    }
                }

                totmin += parseFloat(dhour) * 60 + parseFloat(dtime);
                hour += parseFloat(dhour); 
                time += dtime;
            } else {
                console.log("Distance calculation failed for segment:", a, distance);
            }
        }
        
        // If cal is 0, try to calculate at least a minimum distance
        if (cal == 0 || isNaN(cal)) {
            // Use RadiusCheck or a simple distance calculation as fallback
            // The GetDistance function already handles fallback, so if we get here, 
            // there might be an issue with coordinate parsing
            console.log("Distance calculation returned 0, convertzone:", convertzone);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Unable to calculate distance. Please check your locations or configure Google Maps API key.' });
        }
...
router.post("/add_vehicle_request", async (req, res) => {
    try {
        ...
        // Note: Numeric fields can be 0, so we check for null/undefined specifically
        const requiredFields = {
            "uid": "Customer ID",
            "driverid": "Driver ID array",
            "vehicle_id": "Vehicle ID",
            "price": "Price",
            "tot_km": "Total kilometers",
            "tot_hour": "Total hours",
            "tot_minute": "Total minutes",
        ...
        // Check for missing fields (allowing 0 for numeric values)
        for (const field of Object.keys(requiredFields)) {
            const value = req.body[field];
            // Allow 0 and empty arrays (will be validated separately)
            if (field === "driverid") {
                if (!Array.isArray(value) || value.length === 0) {
                    console.log("Missing field:", field);
                    console.log("Request body:", JSON.stringify(req.body, null, 2));
                    return res.status(200).json({ ResponseCode: 401, Result: false, message: `Missing required field: ${field} (must be non-empty array)` });
                }
            } else if (field === "pickup" || field === "drop") {
                // Allow arrays or strings for location fields
                if (!value || (typeof value !== 'string' && (!Array.isArray(value) || value.length === 0))) {
                    console.log("Missing field:", field);
                    console.log("Request body:", JSON.stringify(req.body, null, 2));
                    return res.status(200).json({ ResponseCode: 401, Result: false, message: `Missing required field: ${field}` });
                }
            } else if (field === "tot_hour" || field === "tot_minute" || field === "price" || field === "tot_km") {
                // Numeric fields: allow 0 (both number and string "0"), but not null/undefined/empty string
                if (value === null || value === undefined || value === "") {
                    console.log("Missing field:", field, "value:", value);
                    console.log("Request body:", JSON.stringify(req.body, null, 2));
                    return res.status(200).json({ ResponseCode: 401, Result: false, message: `Missing required field: ${field} (must be a number, 0 is allowed)` });
                }
                // Check if it's a valid number (including 0)
                // Allow: 0, "0", any valid number
                const numValue = Number(value);
                if (isNaN(numValue)) {
                    console.log("Invalid numeric field:", field, "value:", value);
                    console.log("Request body:", JSON.stringify(req.body, null, 2));
                    return res.status(200).json({ ResponseCode: 401, Result: false, message: `Invalid value for ${field}: must be a number (0 is allowed)` });
                }
            } else if (field === "pickupadd" || field === "dropadd") {
                // Address objects: validate in detail later, just check if they exist as objects
                if (!value || typeof value !== 'object' || Array.isArray(value)) {
                    console.log("Missing or invalid address field:", field, "value:", value);
                    console.log("Request body:", JSON.stringify(req.body, null, 2));
                    return res.status(200).json({ ResponseCode: 401, Result: false, message: `Missing or invalid ${field}. Required: {title, subt} object` });
                }
                // Address objects will be validated in detail after this loop
        ...
        // Validate driverid is an array and not empty
        if (!Array.isArray(driverid) || driverid.length === 0) {
            console.log("Invalid driverid:", driverid);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'driverid must be a non-empty array' });
        }

        // Convert array format to string format if needed
        let pickupStr, dropStr, droplistFormatted = [];
        
        if (Array.isArray(pickup) && pickup.length > 0) {
            pickupStr = `${pickup[0].latitude || pickup[0].lat},${pickup[0].longitude || pickup[0].long}`;
        } else if (typeof pickup === 'string') {
            pickupStr = pickup;
        } else {
            console.log("Invalid pickup format:", pickup);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid pickup format' });
        }

        if (Array.isArray(drop) && drop.length > 0) {
            dropStr = `${drop[0].latitude || drop[0].lat},${drop[0].longitude || drop[0].long}`;
        } else if (typeof drop === 'string') {
            dropStr = drop;
        } else {
            console.log("Invalid drop format:", drop);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid drop format' });
        }

        // Convert droplist from latitude/longitude to lat/long format if needed
        if (droplist && Array.isArray(droplist) && droplist.length > 0) {
            droplistFormatted = droplist.map(item => ({
        ...
        let convertz;
        try {
            convertz = await AllFunction.ZoneLatlon(pickupStr, dropStr, droplistFormatted);
        } catch (zoneError) {
            console.log("ZoneLatlon error:", zoneError);
            console.log("pickupStr:", pickupStr, "dropStr:", dropStr, "droplistFormatted:", droplistFormatted);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Error processing location data' });
        }

        if (!convertz || convertz.length === 0) {
            console.log("ZoneLatlon returned empty result");
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Error processing location coordinates' });
        }
        ...
        // Validate address objects - allow empty subt, just require title
        // Check if pickupadd exists and has a valid title
        if (!pickupadd || typeof pickupadd !== 'object') {
            console.log("Invalid pickupadd - not an object:", pickupadd);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid pickup address format. Required: {title, subt} (subt can be empty)' });
        }
        if (!pickupadd.title || (typeof pickupadd.title !== 'string') || pickupadd.title.trim() === '') {
            console.log("Invalid pickupadd - missing or empty title:", pickupadd);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid pickup address format. Required: {title, subt} (subt can be empty)' });
        }
        
        // Check if dropadd exists and has a valid title
        if (!dropadd || typeof dropadd !== 'object') {
            console.log("Invalid dropadd - not an object:", dropadd);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid drop address format. Required: {title, subt} (subt can be empty)' });
        }
        if (!dropadd.title || (typeof dropadd.title !== 'string') || dropadd.title.trim() === '') {
            console.log("Invalid dropadd - missing or empty title:", dropadd);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Invalid drop address format. Required: {title, subt} (subt can be empty)' });
        }

        // Use empty string if subt is missing, null, or empty - all are acceptable
        const pickupSubt = (pickupadd.subt && typeof pickupadd.subt === 'string') ? pickupadd.subt : '';
        const dropSubt = (dropadd.subt && typeof dropadd.subt === 'string') ? dropadd.subt : '';
        
        picadd = pickupadd.title.trim() + "&!" + pickupSubt; 
        dropad = dropadd.title.trim() + "&!" + dropSubt;
        ...
        if (!vehicle_list || vehicle_list == "" || vehicle_list.length === 0) {
            console.log("Vehicle not found or inactive:", vehicle_id);
            return res.status(200).json({ ResponseCode: 401, Result: false, message: 'Vehicle not found or inactive' });
        }
